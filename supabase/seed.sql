...
('terminology-sotho-tswana', 'ja', 'Sotho-Tswana系言語の用語集', $$Sotho-Tswana系の3言語（Setswana, Sesotho, Sepedi）で、Stellarの主要概念とUI用語をどう表現するかをまとめる作業用ページです。

用語レビューと翻訳のリードはPhenyo Taukobongが担当しています。Setswanaがネイティブ、SesothoとSepediが流暢です。

コア概念用語、研究ワークフロー用語、UI用語の3層で整理します。レジスター、敬体、loanwordの扱いはPhenyoの判断に従います。$$, 'published', now()),
('terminology-sotho-tswana', 'en', 'Terminology — Sotho-Tswana Family', $$A working page for agreeing on how Stellar's core concepts and UI terms are expressed across the Sotho-Tswana language family — Setswana, Sesotho, and Sepedi.

Lead: Phenyo Taukobong, native Setswana speaker with fluent Sesotho and Sepedi. AI developer specialising in low-resource South African languages.

Once a term is agreed here, it propagates to the locale files, the README translations, and the user guide.$$, 'published', now()),
('terminology-sotho-tswana', 'fr', 'Terminologie — Famille Sotho-Tswana', $$Page de travail pour décider comment les concepts de Stellar et les termes d'interface sont exprimés dans la famille Sotho-Tswana — Setswana, Sesotho et Sepedi.

Responsable : Phenyo Taukobong, locuteur natif de Setswana, Sesotho et Sepedi couramment, développeur IA spécialisé dans les langues sud-africaines à faibles ressources.

Une fois un terme validé ici, il se propage aux fichiers de locale, aux traductions du README et au guide utilisateur.$$, 'published', now()),
('terminology-sotho-tswana', 'af', 'Terminologie — Sotho-Tswana-familie', $$Werkblad om ooreen te kom oor hoe Stellar se kernkonsepte en koppelvlak-terme in die Sotho-Tswana-taalfamilie uitgedruk word — Setswana, Sesotho en Sepedi.

Lei: Phenyo Taukobong, Setswana-moedertaalspreker, vlot in Sesotho en Sepedi. KI-ontwikkelaar wat fokus op Suid-Afrikaanse tale met min hulpbronne.

Sodra 'n term hier ooreengekom is, word dit na die locale-lêers, README-vertalings en gebruikersgids versprei.$$, 'published', now())
